...
Once you've downloaded the model weights and placed them into the same directory as the `chat` or `chat.exe` executable, rename them to "chat" (and not Chat_mac for example) then open the terminal, change the directory to where you stored the two files (Chat and .bin) and run:
